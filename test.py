...
from dataset import Graph_Regression_Dataset_test
...
def main(seed):
    ...
    graph_dataset = Graph_Regression_Dataset_test('data/reg/regre.csv', smiles_field='SMILES',
    ...
    for x, adjoin_matrix, y in predct_dataset:
        seq = tf.cast(tf.math.equal(x, 0), tf.float32)
        mask = seq[:, tf.newaxis, tf.newaxis, :]
        preds = model(x, mask=mask, adjoin_matrix=adjoin_matrix, training=False)
