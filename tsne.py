...
df = pd.read_csv('data/reg/data.txt',sep='\t')
ds = Inference_Dataset(df['SMILES'].tolist()).get_data()

medium = {'name': 'Medium', 'num_layers': 6, 'num_heads': 8, 'd_model': 256, 'path': 'medium_weights3', 'addH': True}
arch = medium
trained_epoch = 30
...
atom_to_plot = ['C', 'N', 'O','F', 'S', 'Cl', 'B', 'Br','I','Se','P']
plot_list = []
for aa in atom_to_plot:
    plot_list.append([i.endswith(aa) for i in atom_list])
cmap = [[0.,1.,0.],[0.,0.,1.],[1.,0.,0.],[0.8,0.8,0],[0.7,0.2,0.3],[0.,1.,1.],[0.5,0.6,0.6],[0.6,0.5,1],[0.2,0.2,0.2],[1.0,1.0,0.],[0.5,0.,0.5]]
for j,plot_atom in enumerate(plot_list):
    plt.scatter(Y[plot_atom,0],Y[plot_atom,1],c=np.array(cmap[j]).reshape(1,-1),marker='o',s=3)
...
cmap=[[0.9,0.9,0.8],[0.,1.,1],[1,1,0.], [0.,1,0.],[0.,0.5,0.],[0.5,0.5,0.],[0.5,0.5,0.5],[0.5,0,0],  [1,0.5,0.5],[1,0,0],[0,0.,0.5],[0.5,0.5,1],[0,0.4,1], [0,0,1.0]]
...
keys = ['J','C3','A','B','C','D','E','F','G','H','I']
values = ['60_10C','8_3C','9_3C','50_5C','50_11C','57_2C','57_8C','71_17C','25_12C','54_11C']
for i in range(683):
    atom = atom_list[i]
...
plt.legend(['others','C-N','C-O','C-C','C=C','C=O','Aromatic C','O=','-OH','-O-','N=','-NH2','-NH1','N'],markerscale=12,loc='upper right',labelspacing=1.5)
plt.show()
