...
class Graph_Regression_Dataset_test(object):
    def __init__(self,path,smiles_field='SMILES',label_field='PCE',normalize=False,max_len=1000,addH=True):
        if path.endswith('.txt') or path.endswith('.tsv'):
            self.df = pd.read_csv(path.format('regre'),sep='\t')
        else:
            self.df = pd.read_csv(path.format('regre'))
...
class Graph_Regression_Dataset(object):
    def __init__(self,path,smiles_field='SMILES',label_field='PCE',normalize=False,max_len=1000,addH=True):
        if path.endswith('.txt') or path.endswith('.tsv'):
            ...
            self.df = pd.read_csv(path.format('train3'))
            self.dt = pd.read_csv(path.format('test3'))
    ...
    def get_data(self):
        ...
        self.dataset3 = tf.data.Dataset.from_tensor_slices((data2[self.smiles_field], data2[self.label_field]))
