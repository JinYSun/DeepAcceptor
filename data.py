...
if __name__ == "__main__":
    ...
    f = pd.read_csv (r"J:\screenacc\DATAFOR4.csv")
    re = []
    pce = f['PCE']
    for ind,smile in enumerate ( tqdm(f.iloc[11500:12000,0])):
        print(ind)
        atom,adj = mol_to_geognn_graph_data_MMFF3d(smile)
        np.save('data/reg/val/adj'+str(ind+11500)+'.npy',np.array(adj))
        re.append([atom,'data/reg/val/adj'+str(ind+11500)+'.npy',pce[ind] ])
    r = pd.DataFrame(re)
    r.to_csv('data/reg/val/val11500.csv')
    ...
    # f = pd.read_csv(r'H:\library\NFA-BERT\data\reg/test3.csv')
    # re = []
    # pce = f['PCE']
    ...
    #     atom,adj = mol_to_geognn_graph_data_MMFF3d(smile)
    #     np.save('data/reg/test/'+str(ind+206)+'.npy',np.array(adj))
    #     re.append([atom,'data/pre2/adj'+str(ind+206)+'.npy'])
    # r = pd.DataFrame(re)
    # r.to_csv('data/reg/test/re1.csv')
