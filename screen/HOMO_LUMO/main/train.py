...
if __name__ == "__main__":
    ...
    print('-'*100)
    #(datasets, N_fingerprints) = pp.create_predict(task, 'pre', radius, device)
    ...
    file_result = '../output/result-lumo-' + '.txt'
    if task == 'classification':
    ...
    for epoch in range(iteration):
        ...
        if task == 'regression':
            prediction_dev,prediction = tester.test_regressor(dataset_dev)
            prediction_test,predictions_test = tester.test_regressor(dataset_test)
        ...
        # # prediction_test,predictions_test = tester.test_regressor(datasets)
        # print(predictions_test)
        print(result)
    file_test_result  ='../output/'+ 'lumo_test_prediction'+ '.txt'
    tester.save_model(model,'lumo.h5')
